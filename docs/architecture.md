...
- **Cada academia maneja su propio registro de peleadores, sin compartir datos ni conteos con otras** (decisión explícita del usuario, sesión del 2026-08-06, revirtió el diseño original de esta invariante). `peleador` tiene sus propias `nombres`/`apellidos`/`documento`/`nacimiento`/`sexo`; récord y nivel se calculan solo con el historial que registró ESA organización (`v_mi_peleador`, `security_invoker = true`). `atleta`/`atleta_id` siguen existiendo por dentro (upsert por documento al crear un peleador) solo para que el trigger `registrar_en_historial()` sepa a quién anotarle un resultado y para `peleas_otras_academias()` (cuenta cruzada bajo demanda, sin exponer detalle, botón "Buscar en otras academias") — nunca se expone como concepto ni se usa para autocompletar datos entre academias. `v_resumen_atleta` (el diseño anterior, que sí mezclaba todo) se dejó en el schema sin usar, por si hace falta revisarlo.
