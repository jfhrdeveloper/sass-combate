/* ================= RATE LIMITING — BEST-EFFORT, NO ES LA DEFENSA REAL =================
   La defensa real sería una regla de Vercel Firewall (`vercel firewall rules add ... --action
   rate_limit`), que solo se puede configurar con el proyecto desplegado y enlazado
   (`vercel link`) — el proyecto todavía no tiene hosting decidido, ver docs/pending-task.md.

   Mientras tanto, esto frena el caso obvio (un mismo cliente golpeando /api/pagos/culqi en
   loop) usando un Map en memoria del proceso. Limitación real: en serverless cada instancia
   tiene su propio Map, así que con N instancias corriendo en paralelo el límite efectivo
   puede llegar a N× el configurado — igual vale la pena porque ataja al atacante no
   sofisticado sin depender de infraestructura externa (Redis/Upstash) que este proyecto no
   tiene todavía. */
 ...
 *  en una instancia de larga vida, sin necesitar un `setInterval` de fondo (mala práctica
 *  en serverless: no hay garantía de que la instancia siga viva para dispararlo). */
